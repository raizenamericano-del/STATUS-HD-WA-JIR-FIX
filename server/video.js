...
const SHARPEN_STRENGTH = { '1080p': 0.24, '720p': 0.22, '480p': 0.18, '360p': 0 };
...
function targetDims(profile, meta) {
...
function plan(meta, options = {}) {
  ...
  const wantTrim = opts.trim === 'auto30' || opts.trim === true || opts.trim === '30s';
  ...
  let profile = pickProfile(durationSec, opts.quality);
  const autoQuality = !opts.quality || opts.quality === 'auto';
  const fps = Math.min(ENGINE.maxFps, Math.round(meta.fps || 30));
  ...
  // target ke 720p biar PASTI selesai, daripada gagal kehabisan RAM.
  const ecoDownscale = config.LOW_MEM && heavyEarly && autoQuality && pickProfile(durationSec, 'auto').key === '1080p';
  if (ecoDownscale) profile = PROFILES[1]; // 720p
  const dims = targetDims(profile, meta);

  const audioKbps = meta.hasAudio ? parseInt(ENGINE.audioBitrate, 10) : 0;
  ...
  const capKbps = profile.capMbps * 1000;
  const sizeBudgetKbps = Math.floor((config.MAX_OUTPUT_MB * 1024 * 8) / Math.max(1, durationSec));
  const qualityKbps = mode.twoPass ? capKbps : Math.round(capKbps * 0.62);
...
function audioOptions({ hasAudio, channels, normalize }) {
  if (!hasAudio) return ['-an'];
  const af = normalize ? `-af ${ENGINE.loudnorm}` : null;
  const opts = [
    '-c:a', 'aac',
    '-b:a', ENGINE.audioBitrate,
...
function videoOptions({ vf, crf, bitrateKbps, preset, tune = [], enc = {} }) {
...
function compressVideo(input, output, meta, onProgress, options = {}) {
  ...
  const profile = PROFILES.find((x) => x.key === p.profileKey) || PROFILES[0];
  ...
  const maxBytes = (opts.targetMB || config.MAX_OUTPUT_MB) * 1024 * 1024;
  ...
  const encodeCrf = async (spec) => {
    ...
    const run = runFfmpeg(
      ...
      [
        ...
        '-map', '0:a:0?',
        ...audioOptions({ hasAudio: meta.hasAudio, channels: meta.audioChannels, normalize: config.AUDIO_NORMALIZE }),
      ],
      p.durationSec,
      report,
      {
  ...
  const runLadder = async () => {
    const lowerProfile = PROFILES[Math.min(PROFILES.indexOf(profile) + 1, PROFILES.length - 1)];
    ...
    if (lowerProfile && lowerProfile.key !== profile.key) {
      specs.push(mkSpec({
        ...
        dims: targetDims(lowerProfile, meta),
        preset: 'veryfast',
        enc: leanEnc,
        denoise: false,
        sharpen: 0,
    ...
    for (let i = 0; i < specs.length; i++) {
      ...
      try {
        ...
        if (spec.mode.twoPass) await encodeTwoPass(p.videoKbps, report, spec);
