...
/**
 * FIX PAIRING CODE — normalisasi nama browser:
 * Baileys menurunkan `companion_platform_id` dari nama browser. Di Baileys
 * 7.x nama yang tidak dikenai (mis. "Chrome (Linux)") jatuh ke
 * OTHER_WEB_CLIENT (9) dan server WhatsApp MENOLAK registrasi pairing code
 * dengan 400 bad-request → HP "Gagal menautkan perangkat" (QR tetap lolos,
 * makanya cuma pairing yang rusak). Di 6.7.x ada fallback '1', tapi kita
 * kanonkkan juga biar aman lintas versi & tampilan perangkat tetap wajar
 * (mis. "Chrome (Ubuntu)", sama seperti WA Web asli).
 */
const CANONICAL_BROWSER = [
  ['chrome', 'Chrome'],
  ['edge', 'Edge'],
  ['firefox', 'Firefox'],
  ['opera', 'Opera'],
...
function canonicalBrowserName(raw) {
  const s = String(raw || '').toLowerCase();
  for (const [needle, canon] of CANONICAL_BROWSER) {
    if (s.includes(needle)) return canon;
  }
  if (s === 'ie' || s.includes('internet explorer')) return 'IE';
  if (s === 'desktop') return 'Desktop';
  return 'Chrome'; // default aman-pairing (platform id 1)
}
...
function pickBrowser() {
  ...
  const name = canonicalBrowserName(config.WA_BROWSER_NAME || 'Chrome');
  try {
...
class WhatsAppManager {
  ...
  async start({ forceFresh = false } = {}) {
    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        // Mode pairing butuh waktu lebih lama: user harus buka HP → Perangkat
        // Tertaut → ketik 8 digit. Diperpanjang selama pairing aktif.
        qrTimeout: this.pairingMode ? Math.max(config.QR_TIMEOUT_MS, 180000) : config.QR_TIMEOUT_MS,
  ...
  _onConnUpdate(update) {
    ...
    if (connection === 'open') {
      ...
      this._pairingCodeUnused = false;   // kode berhasil dipakai login
  ...
  _onClose(err) {
    ...
    /* --- Proteksi pairing ---
     * Selama kode pairing masih berlaku, menghapus session = membunuh kode
     * yang sedang menunggu diketik di HP. Jadi saat pairing aktif kita
     * TIDAK menghapus session / menurunkan state ke logged_out — cukup
     * sambung ulang supaya proses pairing bisa diselesaikan WhatsApp. */
    const pairingAlive =
      this.pairingMode &&
      !!this.pairingCode &&
    ...
    if (
      ...
      if (pairingAlive) {
        const sisa = Math.round((this.pairingExpiresAt - Date.now()) / 1000);
        log.warn(
          `Koneksi ditutup (code ${code}) TAPI pairing code masih berlaku ${sisa}s — ` +
          'session DIPERTAHANKAN supaya kode tidak mati. Sambung ulang…'
        );
  ...
  async _requestPairingInternal(clean) {
    ...
    try {
      ...
      // 1) Kode pairing DIGENERATE LOKAL lalu didaftarkan ke server lewat 1 iq:
      //    SATU permintaan per socket, JANGAN retry di socket yang sama —
      //    kode kedua tidak sinkron dengan yang terdaftar di server, jadi
      //    pasti ditolak di HP ("Gagal menautkan perangkat"). Kalau kode
      //    sebelumnya kedaluwarsa tanpa dipakai, atau socket ini sudah pernah
      //    dipakai minta kode → mulai dari session/socket baru.
      ...
      // 4) SATU kali permintaan kode di socket ini (tanpa retry buta)
      this._pairingSentSocket = this.sock;
