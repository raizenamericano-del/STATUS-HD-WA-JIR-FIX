...
app.post('/api/plan', express.json(), async (req, res) => {
  try {
    const { uploadId, quality, mode, trim, trimStartSec, sharpen, denoise } = req.body || {};
    const meta = await probeUpload(uploadId);
    const opts = engineOptions({ quality, mode, trim, trimStartSec, sharpen, denoise });
    const plans = {};
    for (const q of ['auto', '1080p', '720p', '480p', '360p']) {
      plans[q] = videoLib.plan(meta, { ...opts, quality: q });
    }
    res.json({ plan: videoLib.plan(meta, opts), plans });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
