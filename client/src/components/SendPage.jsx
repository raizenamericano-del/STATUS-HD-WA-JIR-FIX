...
export default function SendPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      api('/api/plan', {
        ...
        body: JSON.stringify({ uploadId: up.uploadId, quality, mode, trim: trim ? 'auto30' : 'none', sharpen }),
      })
        .then((res) => {
          if (!alive) return;
          setPlan(res.plan);
  ...
  }, [up?.uploadId, quality, mode, trim, sharpen]);
  ...
  const handleFile = async (f) => {
    ...
    const maxMB = cfg?.maxUploadMB || 300;
    ...
    <div className="space-y-4">
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} className="card p-5 sm:p-7">
        <SectionTitle
          step={1}
          title="Pilih Video"
          subtitle={`Maks ${cfg?.maxUploadMB || 300} MB · MP4/MOV/MKV/AVI/WebM/3GP`}
        />
      ...
      {(upPhase === 'ready' || busy) && phase !== 'success' && (
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="card p-5 sm:p-7">
          <SectionTitle step={3} title="Tujuan & Kirim" subtitle="Kirim ke nomor WA lu sendiri (maks 5 nomor)" />
          ...
          <div className="mt-4 space-y-4">
            ...
            {!busy && (
              <button className="btn-primary w-full !py-4 text-base" onClick={startProcess} disabled={upPhase !== 'ready' || !targetList.length}>
                {phase === 'error' ? <><RefreshCw className="h-5 w-5" /> Coba Lagi</> : <><Send className="h-5 w-5" /> Gas, Encode HD &amp; Kirim!</>}
              </button>
            )}
          </div>
        </motion.div>
