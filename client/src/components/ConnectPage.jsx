    ...
    >
      ...
      <div className="relative">
        <div className="mx-auto mb-4 flex w-fit items-center gap-2 rounded-full border border-white/[0.12] bg-ink-950/50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-300 backdrop-blur">
          <Zap className="h-3 w-3 text-mint" /> {cfg?.engine?.name || 'PureHD Engine'} v{(cfg?.engine?.version || '4.1.0').split('.')[0]}
        </div>
    ...
    <div className="card overflow-hidden p-0">
      <div className="flex items-center gap-2 border-b border-white/[0.06] px-4 py-2.5">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
          {cfg?.engine?.name || 'PureHD Engine'} v{(cfg?.engine?.version || '4.1.0').split('.')[0]} · yang bikin hasilnya tajam
        </span>
      </div>
